...

{
  ...
  IsStartWalk = start_mpc;
		...
		///////normal walking ===============================
          ...
          if (_t_int >=0)
          {
              mpc_body = mpc.step_timing_opti_loop(_t_int, estimated_statex,_Rfoot_location_feedbackx,_Lfoot_location_feedbackx,
                                                   _feedback_lamda,_stop_walking,_t_walkdtime_restart_flag,_t_walkdtime_flag,step_length,step_width);			    
    ...
    _state_generate_interpo(44,0) = mpc_body(15)+ dcm_old(0);  /// next two dcm
    _state_generate_interpo(45,0) = mpc_body(16)+ dcm_old(1);    
