...
class TO_opt:
    ...
    def generate_trajectory(self,detx,detz,bodyvx,bodyvz,q_thigh_ini,q_calf_ini):
        ...
        velX = sol.value(self.velX)[:-1]
        velZ = sol.value(self.velZ)[:-1]


        print(np.hstack((x_pos,z_pos,velX,velZ)))
        print(np.vstack((x_pos,z_pos,velX,velZ)).transpose())
...
if __name__ == '__main__':
    ...
    TO_landing.generate_trajectory(detx = 0.0239,detz = 0.319,bodyvx = 0.43,bodyvz = -2.13,q_thigh_ini=0.647,q_calf_ini=-1.439)
