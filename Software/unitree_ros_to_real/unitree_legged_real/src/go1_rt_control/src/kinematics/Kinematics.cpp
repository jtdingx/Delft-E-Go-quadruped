...
Eigen::Matrix<double, 3,1> Kinematicclass::Forward_kinematics_go1(Eigen::Matrix<double, 3,1> q_joint, int feet_flag)
{   
    if (feet_flag ==0)
    {
        leg_offset_x = FL_leg_offset_x;
...
Eigen::Matrix<double, 3,1> Kinematicclass::Inverse_kinematics(Eigen::Matrix<double, 3,1> pos_des, Eigen::Matrix<double, 3,1> q_ini,int feet_flag)
{
    ...
    for(int j=0; j<10;j++)
    {
        ...
        else
        {
            ...
            if(q_des(0,0)<=UNITREE_LEGGED_SDK::go1_Hip_min)
            {
                q_des(0,0)=UNITREE_LEGGED_SDK::go1_Hip_min;
            }
            else if (q_des(0,0)>=UNITREE_LEGGED_SDK::go1_Hip_max)
            {
                q_des(0,0)=UNITREE_LEGGED_SDK::go1_Hip_max;
            }
            if(q_des(1,0)<=UNITREE_LEGGED_SDK::go1_Thigh_min)
            {
                q_des(1,0)=UNITREE_LEGGED_SDK::go1_Thigh_min;
            }
            else if (q_des(1,0)>=UNITREE_LEGGED_SDK::go1_Thigh_max)
            {
                q_des(1,0)=UNITREE_LEGGED_SDK::go1_Thigh_max;
            }
            if(q_des(2,0)<=UNITREE_LEGGED_SDK::go1_Calf_min)
            {
                q_des(2,0)=UNITREE_LEGGED_SDK::go1_Calf_min;
            }
            else if (q_des(2,0)>=UNITREE_LEGGED_SDK::go1_Calf_max)
            {
                q_des(2,0)=UNITREE_LEGGED_SDK::go1_Calf_max;
            }   
