...
{   
    ...
    force_z_limt = config["fz_limit_ekf"].as<double>();   
    assume_flat_ground = config["assume_flat_ground"].as<double>(); 
     
    std::cout<<"SENSOR_NOISE_ZFOOT:"<<SENSOR_NOISE_ZFOOT<<std::endl;
...
void A1BasicEKF::update_estimation(A1CtrlStates& state, double dt, Eigen::Vector3d estimated_root_pos_offset) {
    ...
    if (state.movement_mode == 0) {  // stand
        ...
        for (int i = 0; i < NUM_LEG; ++i) {
            estimated_contacts[i] = std::min(std::max((state.foot_force(i)) / (2*force_z_limt - 0.0), 0.0), 1.0);
...
//    std::cout << x.transpose() <<std::endl;
    x(0,0) -= estimated_root_pos_offset(0,0);
    x(1,0) -= estimated_root_pos_offset(1,0);
    x(2,0) -= estimated_root_pos_offset(2,0);
