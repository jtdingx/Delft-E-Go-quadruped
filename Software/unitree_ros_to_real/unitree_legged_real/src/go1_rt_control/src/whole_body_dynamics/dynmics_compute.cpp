...
void Dynamiccclass::force_opt(Eigen::Matrix<double, 3,1> base_p,
                              ...
                              Eigen::Matrix<double, 4,1> contact_flag)
{
    ///note: AF = d_total
    Eigen::Matrix<double, 6,12> A;
    A.setZero();
    
    ///// not using initial guess;
    F_leg_guess.setZero();
    ...
    Q_goal = 2 * (A.transpose() * qp_alpha  * A + (qp_beta + qp_gama) * B_unit);
    Q_goal1 = (Q_goal.transpose() + Q_goal) /2.0;


    ///
    q_goal = -2 * (A.transpose() *qp_alpha * FT_total_des + qp_beta * F_leg_guess + qp_gama * grf_opt);
    ...
    if (mode==102) //troting gait pair:FRRL-left; pair:FLRR-right
    {
        ...
        else
        {
            ...
            if(right_support==1) // FR swing leg
            {
                AA.block<3,3>(0,0) = A_unit;
                AA.block<3,3>(9,9) = A_unit;
            }                      
