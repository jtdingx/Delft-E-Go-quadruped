...
int main(int, char **) {
    ...
    state.robot_mass = 13;
    state.a1_trunk_inertia << 0.0168352186, 0.0004636141, 0.0002367952,
                    0.0004636141, 0.0656071082, 3.6671e-05, 
                    0.0002367952, 3.6671e-05,   0.0742720659;


    /// robot state obtained by the state estimator
    ...
    // 0, 1, 2, 3: FL, FR, RL, RR
    state.foot_pos_rel << 0.1881, 0.1881, -0.1881, -0.1881,
                0.12675, -0.12675, 0.12675, -0.12675,
                -0.28, -0.28, -0.28, -0.28;

    /// in this original walking: the assumes a constant desired velocity in the prediction horizon,
    /// in our case, it should be the varying velocity-especially for a jumping motion;;;;
    ...
    /// modifying this!!!!!!, in our case, we used a varying velocity/////////
