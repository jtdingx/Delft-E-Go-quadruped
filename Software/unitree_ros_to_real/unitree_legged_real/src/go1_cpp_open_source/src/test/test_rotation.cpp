...
int main(int, char **) {
    ...
    double roll2 = euler(0), pitch2 = euler(1), yaw2  = euler(1);
    ...
    Eigen::Matrix3d rotat_ma = Utils::eulerAnglesToRotationMatrix(euler);

    //Eigen::Matrix3d romat_4 = Utils::eulerAnglesToRotationMatrix(euler);
    std::cout << "rotation matrix from r p y  is " << std::endl << rotat_ma << std::endl;

    Eigen::Vector3d euler3 = Utils::rotationMatrixToEulerAngles(rotat_ma); 
    std::cout << " rpy form rotation matrix  is " << std::endl << euler3 << std::endl;
