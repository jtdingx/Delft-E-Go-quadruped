...
void A1BasicEKF::update_estimation(A1CtrlStates& state, double dt) {
    ...
    if (state.movement_mode == 0) {  // stand
        ...
        for (int i = 0; i < NUM_LEG; ++i) {
            estimated_contacts[i] = std::min(std::max((state.foot_force(i)) /(gait::force_z_limt*1.5), 0.0), 1.0);
//        estimated_contacts[i] = 1.0/(1.0+std::exp(-(state.foot_force(i)-100)));
        }
    }
    // update Q
    Q.block<3, 3>(0, 0) = PROCESS_NOISE_PIMU  / 1.0 * eye3;
    Q.block<3, 3>(3, 3) = PROCESS_NOISE_VIMU  * 9.8 / 1.0 * eye3;
