...
#include "/home/jiatao/unitree_ros_simu_cvx/src/unitree_ros/go1_rt_control/src/utils/EiQuadProg/EiQuadProg.hpp"
#include <iostream>
...
public:
	...
	virtual void solve(Eigen::MatrixXd & G, const Eigen::VectorXd & g0, const Eigen::MatrixXd & CE, const Eigen::VectorXd & ce0, const Eigen::MatrixXd & CI, const Eigen::VectorXd & ci0, Eigen::VectorXd& X, double &_qp_solver_true) = 0;
...
public:
	...
	void solve(Eigen::MatrixXd & G,
	           ...
	           Eigen::VectorXd& X, double &_qp_solver_true)
	{
		_qpsolver.solve_quadprog(G, g0, CE, ce0, CI, ci0, X,_qp_solver_true);
	};
...
bool QPBaseClass::solveQP()
{
	...
	_qpsolver->solve(_G, _g0, _CE, _ce0, _CI, _ci0, _X,_qp_solver_true);

	if(_qp_solver_true<0.5)
	{
	...
	else
	{
		...
		{
			if (!std::isnan(_X[i])&&(_qp_solver_true>0.5))
			{
